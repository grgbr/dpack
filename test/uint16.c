...
dpackut_uint16_encode(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_encode_uint16(&enc, data->value.uint16),
	                equal,
	                data->error);
...
dpackut_uint16_decode(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_uint16(&dec, &val), equal, data->error);
	if (!data->error) {
...
CUTE_TEST(dpackut_uint16_decode_assert)
{
	uint16_t             val;
	struct dpack_decoder dec = { 0, };
	char                 buff[DPACK_UINT16_SIZE_MAX];
	int                  ret __unused;
...
{
	/* 65536 */
	DPACKUT_UINT16(data, "\xce\x00\x01\x00\x00", -ERANGE, UINT16_C(0));
	dpackut_uint16_decode(&data);
}
...
dpackut_uint16_decode_min(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_uint16_min(&dec, data->low.uint16, &val),
	                equal,
	                data->error);
	if (!data->error) {
		cute_check_uint(val, equal, data->value.uint16);
...
{
	/* 32767 */
	DPACKUT_UINT16_MIN(data, "\xcd\x7f\xff", 0, UINT16_MAX / 2, UINT16_C(1));
	dpackut_uint16_decode_min(&data);
}
...
{
	/* 65536 */
	DPACKUT_UINT16_MIN(data, "\xce\x00\x01\x00\x00", -ERANGE, 0, UINT16_C(1));
	dpackut_uint16_decode_min(&data);
}
...
{
	/* 32767 */
	DPACKUT_UINT16_MIN(data, "\xcd\x7f\xff", 0, UINT16_MAX / 2, UINT16_MAX / 2);
	dpackut_uint16_decode_min(&data);
}
...
dpackut_uint16_decode_max(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_uint16_max(&dec, data->high.uint16, &val),
	                equal,
	                data->error);
	if (!data->error) {
		cute_check_uint(val, equal, data->value.uint16);
...
dpackut_uint16_decode_range(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_uint16_range(&dec,
	                                         data->low.uint16,
	                                         data->high.uint16,
	                                         &val),
	                equal,
