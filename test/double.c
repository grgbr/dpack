...
#if 0
#define DPACKUT_DOUBLE_MAX(_packed, _error, _value, _high) \
	{ \
		.packed     = _packed, \
		.size       = sizeof(_packed) - 1, \
		.error      = _error, \
...
dpack_scalar_utest_decode_double_max(void ** state __unused)
{
	static const struct dpack_scalar_utest_data data[] = {
		...
		/* +Not A Number: dpack-utest-gen.py -s "float('nan')" */
		DPACKUT_DOUBLE_MAX("\xca\x7f\xc0\x00\x00", -ENOMSG, NAN,       0.0),
		/* +0 (float): dpack-utest-gen.py -s "0.0" */
		DPACKUT_DOUBLE_MAX("\xca\x00\x00\x00\x00", -ENOMSG, 0.0,       0.0)
	};
	...
	cute_expect_assertion(ret = dpack_decode_double_max(&dec, -INFINITY, &val));
	cute_expect_assertion(ret = dpack_decode_double_max(&dec, INFINITY, &val));
...
#define DPACKUT_DOUBLE_RANGE(_packed, _error, _value, _low, _high) \
	{ \
		.packed     = _packed, \
		.size       = sizeof(_packed) - 1, \
		.error      = _error, \
...
dpack_scalar_utest_decode_double_range(void ** state __unused)
{
	static const struct dpack_scalar_utest_data data[] = {
		...
		/* +Not A Number: dpack-utest-gen.py -s "float('nan')" */
		DPACKUT_DOUBLE_RANGE("\xca\x7f\xc0\x00\x00", -ENOMSG, NAN,       0.0,      1.0),
		/* +0 (float): dpack-utest-gen.py "0.0" */
		DPACKUT_DOUBLE_RANGE("\xca\x00\x00\x00\x00", -ENOMSG, 0.0,       0.0,      1.0)
	};
...
#if 0
	CUTE_REF(dpack_scalar_utest_decode_double),
	CUTE_REF(dpack_scalar_utest_decode_double_min),
	CUTE_REF(dpack_scalar_utest_decode_double_max),
	CUTE_REF(dpack_scalar_utest_decode_double_range)
#endif
};
