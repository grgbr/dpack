...
dpackut_uint8_encode(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_encode_uint8(&enc.base, data->value.uint8),
	                equal,
	                data->error);
...
dpackut_uint8_decode(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_uint8(&dec.base, &val),
	                equal,
	                data->error);
	if (!data->error) {
...
CUTE_TEST(dpackut_uint8_decode_assert)
{
	uint8_t                     val;
	struct dpack_decoder_buffer dec = { 0, };
	uint8_t                     buff[DPACK_UINT8_SIZE_MAX];
	int                         ret __unused;

	cute_expect_assertion(ret = dpack_decode_uint8(NULL, &val));
#if defined(CONFIG_DPACK_DEBUG)
	cute_expect_assertion(ret = dpack_decode_uint8(&dec.base, &val));
#endif /* defined(CONFIG_DPACK_DEBUG) */

	dpack_decoder_init_buffer(&dec, buff, sizeof(buff));
	cute_expect_assertion(ret = dpack_decode_uint8(&dec.base, NULL));
	dpack_decoder_fini(&dec.base);
}
...
{
	/* 256 */
	DPACKUT_UINT8(data, "\xcd\x01\x00", -ENOMSG, 0);
	dpackut_uint8_decode(&data);
}
...
dpackut_uint8_decode_min(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_uint8_min(&dec.base,
	                                       data->low.uint8,
	                                       &val),
	                equal,
	                data->error);
...
#if defined(CONFIG_DPACK_DEBUG)
	cute_expect_assertion(ret = dpack_decode_uint8_min(&dec.base, 1, &val));
#endif /* defined(CONFIG_DPACK_DEBUG) */

	dpack_decoder_init_buffer(&dec, buff, sizeof(buff));
	cute_expect_assertion(ret = dpack_decode_uint8_min(&dec.base, 0, &val));
	cute_expect_assertion(ret = dpack_decode_uint8_min(&dec.base,
	                                                   UINT8_MAX,
	                                                   &val));
	cute_expect_assertion(ret = dpack_decode_uint8_min(&dec.base, 1, NULL));
	dpack_decoder_fini(&dec.base);
}
...
{
	/* 256 */
	DPACKUT_UINT8_MIN(data, "\xcd\x01\x00", -ENOMSG, 0, UINT8_C(1));
	dpackut_uint8_decode_min(&data);
}
...
{
	/* 256 */
	DPACKUT_UINT8_MIN(data, "\xcd\x01\x00", -ENOMSG, 0, UINT8_C(254));
	dpackut_uint8_decode_min(&data);
}
...
dpackut_uint8_decode_max(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_uint8_max(&dec.base, data->high.uint8, &val),
	                equal,
	                data->error);
	if (!data->error) {
		cute_check_uint(val, equal, data->value.uint8);
...
#if defined(CONFIG_DPACK_DEBUG)
	cute_expect_assertion(ret = dpack_decode_uint8_max(&dec.base, 1, &val));
#endif /* defined(CONFIG_DPACK_DEBUG) */

	dpack_decoder_init_buffer(&dec, buff, sizeof(buff));
	cute_expect_assertion(ret = dpack_decode_uint8_max(&dec.base, 0, &val));
	cute_expect_assertion(ret = dpack_decode_uint8_max(&dec.base,
	                                                   UINT8_MAX,
	                                                   &val));
	cute_expect_assertion(ret = dpack_decode_uint8_max(&dec.base, 1, NULL));
	dpack_decoder_fini(&dec.base);
}
...
{
		/* 256 */
		DPACKUT_UINT8_MAX(data, "\xcd\x01\x00", -ENOMSG, 0, UINT8_C(1));
		dpackut_uint8_decode_max(&data);
}
...
dpackut_uint8_decode_range(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_uint8_range(&dec.base,
	                                         data->low.uint8,
	                                         data->high.uint8,
	                                         &val),
	                equal,
...
CUTE_TEST(dpackut_uint8_decode_range_assert)
{
	...
	cute_expect_assertion(ret = dpack_decode_uint8_range(NULL,
	                                                     1,
	                                                     2,
	                                                     &val));
#if defined(CONFIG_DPACK_DEBUG)
	cute_expect_assertion(ret = dpack_decode_uint8_range(&dec.base,
	                                                     1,
	                                                     2,
	                                                     &val));
#endif /* defined(CONFIG_DPACK_DEBUG) */
