...
dpackut_int64_encode(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_encode_int64(&enc.base, data->value.int64),
	                equal,
	                data->error);
...
CUTE_TEST(dpackut_int64_encode_assert)
{
	...
	cute_expect_assertion(ret = dpack_encode_int64(&enc.base, val));
...
dpackut_int64_decode(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_int64(&dec.base, &val), equal, data->error);
	if (!data->error) {
...
CUTE_TEST(dpackut_int64_decode_assert)
{
	int64_t                     val;
	struct dpack_decoder_buffer dec = { 0, };
	uint8_t                     buff[DPACK_INT64_SIZE_MAX];
	int                         ret __unused;

	cute_expect_assertion(ret = dpack_decode_int64(NULL, &val));
#if defined(CONFIG_DPACK_DEBUG)
	cute_expect_assertion(ret = dpack_decode_int64(&dec.base, &val));
#endif /* defined(CONFIG_DPACK_DEBUG) */

	dpack_decoder_init_buffer(&dec, buff, sizeof(buff));
	cute_expect_assertion(ret = dpack_decode_int64(&dec.base, NULL));
	dpack_decoder_fini(&dec.base);
}
...
dpackut_int64_decode_min(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_int64_min(&dec.base, data->low.int64, &val),
	                equal,
	                data->error);
	if (!data->error) {
		cute_check_sint(val, equal, data->value.int64);
...
CUTE_TEST(dpackut_int64_decode_min_assert)
{
...
#if defined(CONFIG_DPACK_DEBUG)
	cute_expect_assertion(ret = dpack_decode_int64_min(&dec.base, 1, &val));
#endif /* defined(CONFIG_DPACK_DEBUG) */
	cute_expect_assertion(ret = dpack_decode_int64_min(&dec.base,
	                                                   INT64_MIN,
	                                                   &val));
	cute_expect_assertion(ret = dpack_decode_int64_min(&dec.base,
	                                                   INT64_MAX,
	...
	dpack_decoder_init_buffer(&dec, buff, sizeof(buff));
	cute_expect_assertion(ret = dpack_decode_int64_min(&dec.base, 1, NULL));
	dpack_decoder_fini(&dec.base);
}
...
dpackut_int64_decode_max(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_int64_max(&dec.base, data->high.int64, &val),
	                equal,
	                data->error);
	if (!data->error) {
		cute_check_sint(val, equal, data->value.int64);
...
#if defined(CONFIG_DPACK_DEBUG)
	cute_expect_assertion(ret = dpack_decode_int64_max(&dec.base, 1, &val));
#endif /* defined(CONFIG_DPACK_DEBUG) */

	dpack_decoder_init_buffer(&dec, buff, sizeof(buff));
	cute_expect_assertion(ret = dpack_decode_int64_max(&dec.base,
	                                                   INT64_MIN,
	                                                   &val));
	cute_expect_assertion(ret = dpack_decode_int64_max(&dec.base,
	                                                   INT64_MAX,
	                                                   &val));
	cute_expect_assertion(ret = dpack_decode_int64_max(&dec.base, 1, NULL));
	dpack_decoder_fini(&dec.base);
}
...
dpackut_int64_decode_range(const struct dpackut_scalar_data * data)
{
	...
	cute_check_sint(dpack_decode_int64_range(&dec.base,
	                                         data->low.int64,
	                                         data->high.int64,
	                                         &val),
	                equal,
...
#if defined(CONFIG_DPACK_DEBUG)
	cute_expect_assertion(ret = dpack_decode_int64_range(&dec.base, 1, 2, &val));
#endif /* defined(CONFIG_DPACK_DEBUG) */

	dpack_decoder_init_buffer(&dec, buff, sizeof(buff));
	cute_expect_assertion(ret = dpack_decode_int64_range(&dec.base,
	                                                     INT64_MIN,
	                                                     2,
	                                                     &val));
	cute_expect_assertion(ret = dpack_decode_int64_range(&dec.base,
	                                                     ...
	                                                     INT64_MAX,
	                                                     &val));
	cute_expect_assertion(ret = dpack_decode_int64_range(&dec.base, 2, 2, &val));
	cute_expect_assertion(ret = dpack_decode_int64_range(&dec.base, 1, 2, NULL));
	dpack_decoder_fini(&dec.base);
}
